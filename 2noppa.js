...
function syotteenTarkistus(event) {
    ...
    if(pelaaja1 === "" && pelaaja2 !== "" && pelaaja3 === "" && pelaaja4 === ""){
        ...
        location.reload("2noppapeli.html")
    }
    if(pelaaja1 === "" && pelaaja2 === "" && pelaaja3 !== "" && pelaaja4 === ""){
        alert("Pelaajat tulee syöttää järjestyksessä vasemmalta oikealla.")
        location.reload("2noppapeli.html")
    ...
    if (saavutettavaPistemaara < 40) {
        alert("Minimipistemäärä on 40.")
        return;
...
//let sattuma2 = Math.floor(Math.random() * 6) + 1;;

function nopanHeitto() {

    let tuloste = document.getElementById("pistetila");
    ...
    let kuvahaku2 = "grafiikat/" + kuvat[sattuma2 - 1];
    ...
    tulostus.appendChild(kuva2);

    if (sattuma === 1 && sattuma2 === 1) {
        pisteet += 25;
    } else if (sattuma === 1 || sattuma2 === 1) {
        tuloste.textContent = 0;
        pisteet = 0;
        pisteet2 = 0;
        pelivuoro = (pelivuoro % pelaajaMaara) + 1;
        vuoro.textContent = "Pelaajan " + pelivuoro + " vuoro";
        tuplamaara = 0; 
    } else if (sattuma === sattuma2) {
        pisteet += sattuma * 3;
        tuplamaara++;
        if (tuplamaara === 3) {
            tuloste.textContent = 0;
...
function pisteidenLisays() {
    ...
    let pisteeet = pisteet + pisteet2;
    
    
    let pistelisays = document.getElementById("pistetila");
    pistelisays.textContent = pisteeet;
...
function lopetaHeitto() {
    ...
    let pistelisays = document.getElementById("pistetila");
    let pelaajanPisteet;
    let yhteispisteet = pisteet + pisteet2;
